...
router.patch('/me/profile', requireAuth, (req, res) => {
  ...
  let { bio, status, avatar_url, banner_url } = req.body;
  ...
  db.prepare('UPDATE users SET bio = ?, status = ?, avatar_url = ?, banner_url = ? WHERE id = ?').run(
    bio ?? null, status ?? null, avatar_url ?? null, banner_url ?? null, uid
  );

  const updated = db.prepare('SELECT id, username, role, bio, status, avatar_url, banner_url FROM users WHERE id = ?').get(uid);
  res.json(updated);
});
...
router.get('/dm/conversations', requireAuth, (req, res) => {
  ...
  const convs = db.prepare(`
    SELECT
      ...
      u.username AS partner_username, u.role AS partner_role,
